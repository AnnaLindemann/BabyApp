...
window.addEventListener("DOMContentLoaded", () => {
  ...
  function renderEventsForDate(date) {
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    // очистить текущий список на экране
    eventList.innerHTML = "";

    // взять все сохранённые события
    const events = JSON.parse(localStorage.getItem("events") || "[]");
    // фильтровать только по этой дате
  ...
  globalDateInput.value = today; // показываем сегодня в календаре
  renderEventsForDate(today); // отрисовываем список за сегодня
  updateFeedTotal();
  ...
  function scheduleMidnightRefresh() {
    ////!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    const now = new Date();
    // создаём объект «завтра в 00:00»
    const next = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1);
    const ms = next - now; // миллисекунд до полуночи

    setTimeout(() => {
      // сбрасываем календарь (пустая строка ⇒ today внутри getDateOrToday)
      globalDateInput.value = "";
      const today = getDateOrToday();
      renderEventsForDate(today);
      updateFeedTotal();

      // планируем следующий запуск уже для следующей полуночи
